"""Dagster asset: CarParkAvailability snapshot -> ONE opaque raw row -> BigQuery (WRITE_APPEND).

Deliberately near-identical to ev_charger_availability / traffic_speed_bands asset.py: same
4-column RAW_SCHEMA, same CREATE_IF_NEEDED load, same batch_id/ingested_at stamping. Only
source_name, the fetch call, and the metadata count differ. The duplication is intentional
(rule of three) — with all three sources side by side it is the evidence for the config-driven
factory (concern #1). `GCP_PROJECT_ID` + `BQ_DATASET_RAW` are the dev/prod switch.
"""
...
from .fetch import fetch_carpark_availability

SOURCE_NAME = "carpark_availability"
...
@asset(
    ...
    description="LTA CarParkAvailability snapshot landed opaquely (whole payload) into raw.",
    # A transient DNS/connection blip on a scheduled tick would lose that snapshot; ride
    # it out within the run (same stance as the other ingestion assets). Dagster does not
...
def carpark_availability(context: AssetExecutionContext) -> MaterializeResult:
    api_key = os.environ["LTA_API_KEY"]
    project = os.environ["GCP_PROJECT_ID"]
    dataset = os.environ["BQ_DATASET_RAW"]

    batch_id = context.run.run_id
    snapshot = fetch_carpark_availability(api_key)
    row = {
        "batch_id": batch_id,
